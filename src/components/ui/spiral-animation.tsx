...
class AnimationController {
    ...
    private readonly numberOfStars = 9000
    private readonly trailLength = 80

    constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number, scrollDriven = false) {
        this.canvas = canvas
        this.ctx = ctx
        this.dpr = dpr
        this.size = size
        ...
        // Scroll-driven mode: no auto-play loop — the parent scrubs `time` via
        // renderAt() so the starfield stays locked to the scroll position.
        if (!scrollDriven) this.setupTimeline()
    }

    /** Render one frame at a given animation time (0–1) — used for scroll scrubbing. */
    public renderAt(t: number) {
        this.time = t
        this.render()
    }
...
export function SpiralAnimation({
    ...
    // on mount (see the resize effect below).
    ...
    useEffect(() => {
        ...
        // 设置CSS尺寸 — keep it SQUARE (matches the square buffer) and centred via
        // the classes below so the spiral stays circular and covers the section,
        // instead of stretching into an ellipse in a wide (non-square) container.
